...
describe("createOpenAIClient", () => {
  ...
  it("completes normally without retry", async () => {
    const originalFetch = globalThis.fetch;
    let calls = 0;
    globalThis.fetch = (async () => {
      ...
      return {
        ok: true,
        json: async () => ({
          choices: [{ message: { content: "once", tool_calls: [] } }],
        }),
      } as any;
    }) as any;
    try {
  ...
  it("does not retry non-retryable 4xx (400/401)", async () => {
    ...
    for (const status of [400, 401]) {
      ...
      ).rejects.toThrow(new RegExp(`Model request failed: ${status}`));
      expect(calls).toBe(1);
    }
    globalThis.fetch = originalFetch;
  });
  ...
  it("retries 429 on streaming path then streams deltas", async () => {
    const originalFetch = globalThis.fetch;
    let calls = 0;
    const chunks = [
  ...
  it("times out mid-stream without returning partial success", async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (async (_url: any, init: any) => {
      const signal = init?.signal as AbortSignal | undefined;
      const encoder = new TextEncoder();
    ...
    try {
      ...
      // may have emitted a partial delta, but must not resolve successfully
